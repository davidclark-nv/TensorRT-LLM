...
std::vector<torch::Tensor> run_fp4_block_scale_moe_runner(torch::Tensor const& routing_logits,
    ...
    int64_t const num_experts, int64_t const top_k, int64_t const num_fused_shared_expert,
    ...
    bool const do_finalize, MoeRunnerType& moe_runner, int64_t const moeConfigIndex)
{
...
public:
    ...
    [[nodiscard]] std::vector<torch::Tensor> run(torch::Tensor const& routing_logits,
        ...
        torch::Tensor const& gemm2_weights_scale, torch::Tensor const& output1_scales_scalar,
        torch::Tensor const& output1_scales_gate_scalar, torch::Tensor const& output2_scales_scalar,
        int64_t const num_experts, int64_t const top_k, int64_t num_fused_shared_expert,
        std::optional<int64_t> const n_group, std::optional<int64_t> const topk_group, int64_t const intermediate_size,
        int64_t const local_expert_offset, int64_t const local_num_experts,
        ...
        int64_t moeConfigIndex)
    {
        ...
        return run_fp4_block_scale_moe_runner(routing_logits, routing_bias, hidden_states, hidden_states_scale,
            ...
            output1_scales_gate_scalar, output2_scales_scalar, num_experts, top_k, num_fused_shared_expert, n_group,
            topk_group, intermediate_size, local_expert_offset, local_num_experts, routed_scaling_factor,
            mTileTokensDim, routing_method_type, do_finalize, *mRunner, moeConfigIndex);
    }
